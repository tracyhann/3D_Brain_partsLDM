...
CONFIG="${CONFIG:-${CONFIG_PATH:-configs/rhemi_AE_spacing1p5.json}}"
...
OUT_PREFIX="${OUT_PREFIX:-${CFG_PREFIX:-rhemi_AE}}"
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/train_ae_rhemi_ddp_${SLURM_JOB_ID:-manual}_node${NODE_RANK}_$(date +%Y%m%d_%H%M%S).log"
