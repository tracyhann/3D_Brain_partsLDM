"""
DDP trainer for train_3d_brain_ldm_aux_taux.py (Aux-tAux whole-brain LDM).

Example (8 GPUs):
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --standalone --nnodes=1 --nproc_per_node=8 \
  scripts/train_3d_brain_ldm_aux_taux_ddp.py \
    ...
    *,
    ...
    if whole_scale_factor is None:
        with torch.no_grad(), torch.amp.autocast("cuda", enabled=amp_enabled):
            ...
            z = whole_ae.encode_stage_2_inputs(batch0["image"].to(device, non_blocking=True))
    ...
    if hemi_scale_factor is None or sub_scale_factor is None:
        batch0 = next(iter(train_loader))
        ref_img = batch0["image"].to(device, non_blocking=True)[:1]
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="DDP wrapper for train_3d_brain_ldm_aux_taux.py.",
        parents=[pre_ap],
    )
    ...
    budget_after = None
    budget_ratio = 1.0
    if parse_bool(args.auto_scale_budget_by_batch):
        ...
        budget_before = {k: int(getattr(args, k)) for k in step_keys}
        for key in step_keys:
            setattr(args, key, base._scale_step_value(int(getattr(args, key)), budget_ratio))
        budget_after = {k: int(getattr(args, k)) for k in step_keys}
    ...
    try:
        ...
        base.seed_all(
            int(args.seed) + rank,
            deterministic=parse_bool(args.deterministic),
            allow_tf32=parse_bool(args.allow_tf32),
        )
        random.seed(int(args.seed) + rank)

        if rank == 0 and budget_before is not None and budget_after is not None:
            print(
            ...
            print(
                ...
                + ", ".join([f"{k}:{budget_before[k]}->{budget_after[k]}" for k in budget_before.keys()])
            )
        ...
        train_loader, val_loader, test_loader, train_sampler = make_dataloaders_from_csv_ddp(
            ...
            persistent_workers=parse_bool(args.dataloader_persistent_workers),
