...
def sample_ldm_proto1(
    ...
    xL_int_lat = F.interpolate(xL_int, size=(latent_shape[-3], latent_shape[-2], latent_shape[-1]),
                        mode="trilinear", align_corners=False).to(device)
    xR_int_lat = F.interpolate(xR_int, size=(latent_shape[-3], latent_shape[-2], latent_shape[-1]),
                        mode="trilinear", align_corners=False).to(device)
    xC_int_lat = F.interpolate(xC_int, size=(latent_shape[-3], latent_shape[-2], latent_shape[-1]),
                        mode="trilinear", align_corners=False).to(device)
    ...
    cond_lat = torch.cat([xL_int_lat, xR_int_lat, xC_int_lat], dim=1).to(device)  # [B,6,12,16,12]
    ...
    for i, t in enumerate(timesteps):
        ...
        model_in = torch.cat([z, cond_lat], dim=1) # [B, 3+2, 12,16,12]n
    ...
    for epoch in range(n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            with torch.autocast("cuda", enabled=torch_autocast):
                ...
                # Sample timestep + noise
                xL_int_lat = F.interpolate(xL_int, size=(z_gt.shape[-3], z_gt.shape[-2], z_gt.shape[-1]),
                                    mode="trilinear", align_corners=False).to(device)
                xR_int_lat = F.interpolate(xR_int, size=(z_gt.shape[-3], z_gt.shape[-2], z_gt.shape[-1]),
                                    mode="trilinear", align_corners=False).to(device)
                xC_int_lat = F.interpolate(xC_int, size=(z_gt.shape[-3], z_gt.shape[-2], z_gt.shape[-1]),
                                    mode="trilinear", align_corners=False).to(device)
        ...
        if epoch_loss / (step + 1) < unet_best_loss:
            ...
            sample_ldm_proto1(train_loader, autoencoder, unet, scheduler, device, latent_shape = latent_shape,
...
def main():
    ...
    unet = DiffusionModelUNet(
        spatial_dims=3,
        in_channels=3+3, # default: 3 + num cond channel
