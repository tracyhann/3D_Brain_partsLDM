...
from pair_inferer import LatentDiffusionInferer
...
def sample_ldm_conditioner(
    ...
    with torch.no_grad():
        ...
        if pairs is not None:
            ...
            z_pairs = torch.zeros(latent_shape, device=device)  # dummy
...
def main():
    ...
    def keep_left_half(x):
        mid = x.shape[-3] // 2          # assumes x is (C, D, H, W) or (D,H,W) with D as first spatial dim
        return x[:, mid:, :, :]

    channel = 0  # 0 = Flair
    keys = ['image', 'mask', 'pair', 'pair_mask']
    train_transforms = transforms.Compose(
        [
        transforms.LoadImaged(keys=keys),
        transforms.EnsureChannelFirstd(keys=keys),
        transforms.Lambdad(keys=keys, func=keep_left_half),
    ...
    unet = DiffusionModelUNet(
        spatial_dims=3,
        in_channels=2*ae_latent_ch, # default: 3
