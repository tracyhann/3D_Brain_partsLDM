"""
DDP trainer for context-conditioned Aux-tAux whole-brain LDM.

Example (8 GPUs):
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --standalone --nnodes=1 --nproc_per_node=8 \
  scripts/train_3d_brain_ldm_aux_taux_context_ddp.py \
  --config configs/whole_brain_aux_taux_spacing1p5.json
"""
...
def _build_ctx_input(z_t: torch.Tensor, z_ctx0: Optional[torch.Tensor]) -> torch.Tensor:
    if z_ctx0 is None:
        z_ctx = torch.zeros_like(z_t)
    else:
        z_ctx = z_ctx0
        if z_ctx.shape[0] != z_t.shape[0]:
            if z_ctx.shape[0] == 1:
                z_ctx = z_ctx.expand(z_t.shape[0], -1, -1, -1, -1)
            else:
                raise ValueError(f"Context batch mismatch: z_t={z_t.shape}, z_ctx={z_ctx.shape}")
    return torch.cat([z_t, z_ctx], dim=1)


def _predict_eps_with_ctx(
    *,
    unet: torch.nn.Module,
    z_t: torch.Tensor,
    timesteps: torch.Tensor,
    z_ctx0: Optional[torch.Tensor],
    amp_enabled: bool,
) -> torch.Tensor:
    model_input = _build_ctx_input(z_t, z_ctx0)
    with torch.amp.autocast("cuda", enabled=amp_enabled):
        return unet(model_input, timesteps=timesteps)


@torch.no_grad()
def _reverse_ddpm_to_t0_with_ctx(
    *,
    unet: torch.nn.Module,
    ...
    for t in range(int(t_start), -1, -1):
        ...
        z = base._step_prev_sample(scheduler.step(model_output=eps, timestep=t, sample=z))
    return z


@torch.no_grad()
def eval_ldm_loss_only_context(
    *,
...
def eval_taux_fast_context(
    *,
    ...
    for i in range(eval_n):
        ...
        z_final0 = _reverse_ddpm_to_t0_with_ctx(
            unet=whole_unet,
            scheduler=ddpm,
            z_t=payload["z_whole_taux"],
            z_ctx0=payload["z_coarse0"],
            t_start=t_aux,
            amp_enabled=amp_enabled,
        )
    ...
    *,
    ...
    if hemi_scale_factor is None or sub_scale_factor is None:
        ...
        if hemi_scale_factor is None:
            local_hemi_sf = base._estimate_scale_factor_from_batch(
                ae=hemi_ae,
                x=torch.cat([xL, xR], dim=0),
                amp_enabled=amp_enabled,
            )
            hemi_scale_factor = reduce_mean_scalar(local_hemi_sf, device=device, world_size=world_size)
            if rank == 0:
    ...
    for step in range(global_step, max_steps):
        ...
        if do_aux:
            ...
            eps_pred = _predict_eps_with_ctx(
                unet=ddp_whole_unet,
                z_t=z_t,
                timesteps=timesteps,
                z_ctx0=z_ctx0,
        ...
        if full_eval_trigger:
            if rank == 0 and val_loader is not None and len(val_loader) > 0:
                metrics = eval_taux_fast_context(
                    whole_unet=ddp_whole_unet.module,
                    whole_ae=whole_ae,
                    hemi_ae=hemi_ae,
                    hemi_unet=hemi_unet,
            ...
            if rank == 0 and val_loader is not None and len(val_loader) > 0:
                metrics = eval_ldm_loss_only_context(
                    whole_unet=ddp_whole_unet.module,
                    whole_ae=whole_ae,
                    val_loader=val_loader,
                    device=device,
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="DDP wrapper for context-conditioned train_3d_brain_ldm_aux_taux.py.",
        parents=[pre_ap],
    )
    ...
    ap.add_argument("--out_prefix", default="whole_brain_aux_taux_context_UNET")
    ...
    try:
        ...
        whole_unet = DiffusionModelUNet(
            spatial_dims=3,
            # z_t concatenated with z_coarse context.
            in_channels=int(args.whole_ae_latent_ch) * 2,
