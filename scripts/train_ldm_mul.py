...
# ======================
# LDM training (DDP-aware, AE frozen, UNet multi-GPU)
# ======================
def train_ldm(
    unet,
    train_loader,
    ...
    # ---- unwrap AE if wrapped in DDP (for stage='both' case) ----
    if isinstance(autoencoder, DDP):
        ae_model = autoencoder.module
    else:
        ae_model = autoencoder

    # ---- unwrap UNet for attribute access (but training uses outer unet DDP) ----
    if isinstance(unet, DDP):
        unet_model = unet.module
    else:
        unet_model = unet
    ...
    with torch.no_grad():
        with autocast(device_type="cuda", enabled=(device.type == "cuda")):
            ...
            z_check = ae_model.encode_stage_2_inputs(check_img)
    ...
    if unet_model.in_channels != expected_in_channels:
        raise ValueError(
            f"UNet in_channels={unet_model.in_channels}, "
    ...
    for epoch in range(ldm_epochs):
        ...
        for step, batch in progress_bar:
            ...
            with autocast(device_type="cuda", enabled=(device.type == "cuda")):
                with torch.no_grad():
                    latents = ae_model.encode_stage_2_inputs(images)
...
def main():
    ...
    # ---- sanity: stage=ldm 必须提供 ckpt ----
    if args.stage == "ldm" and (args.ae_ckpt is None or args.ae_ckpt == ""):
        if is_main_process(rank):
            raise ValueError("For stage='ldm', you must provide --ae_ckpt path to a pretrained AE_best.pt.")
        else:
            return
    ...
    # load pretrained AE if provided
    if args.ae_ckpt != "":
    ...
    # wrap AE in DDP **only** when we are going to train it
    if world_size > 1 and args.stage in ["ae", "both"]:
        autoencoder = DDP(
            autoencoder,
    ...
    unet = DiffusionModelUNet(
        spatial_dims=3,
        in_channels=ae_latent_ch + ldm_num_parts,  # latent_channels + cond
