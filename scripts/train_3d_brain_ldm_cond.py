...
from eval_utils import psnr, ssim_3d, fid_from_features, plot_recon_loss, plot_adversarial_loss, plot_reconstructions, sample_ldm, sample_ldm_cond, plot_unet_loss
...
def make_dataloaders_from_csv(csv_path, conditions = ['age', 'sex', 'vol', 'part'], train_transforms = None,
...
def train_ldm(unet, train_loader, autoencoder, ldm_epochs=150, ldm_num_parts=5,
    lr=1e-4,device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    outdir="ckpts",sample_every=25,):

    # 1) Scheduler (same as before)
    ...
    # 2) Compute scale_factor on one batch (unconditional AE)
    with torch.no_grad():
    ...
    # Optional: you can still create an inferer for later sampling
    ...
    for epoch in range(ldm_epochs):
        ...
        for step, batch in progress_bar:
            ...
            part_id = batch["part"].to(device)      # [B], long
            ...
            with autocast(device_type="cuda", enabled=(device.type == "cuda")):
                # 4) Encode to latents (unconditional AE)
                with torch.no_grad():
                    latents = autoencoder.encode_stage_2_inputs(images)  # [B, Cz, Dz, Hz, Wz]
                    latents = latents * scale_factor

                # 5) Sample noise & timesteps
                ...
                # 6) Forward diffusion: add noise to latents
        ...
        if (epoch + 1) % sample_every == 0:
            for i in range(ldm_num_parts):
                sample_ldm_cond(train_loader, autoencoder, unet, scheduler, device, idx = 0, channel = 0, 
                        outdir = outdir, filename = f'synthetic_ep{epoch+1}_part{i}', part_id=i)
...
def main():
    ...
    train_loader, val_loader = make_dataloaders_from_csv(args.csv, conditions = ['age', 'sex', 'vol', 'part'], train_transforms = train_transforms,
    ...
    unet = DiffusionModelUNet(
        spatial_dims=3,
        in_channels=3 + ldm_num_parts, # default: 3
    ...
    if args.stage in ["ldm", "both"]:
        train_ldm(unet, train_loader, autoencoder, ldm_epochs=args.ldm_epochs, ldm_num_parts=ldm_num_parts,
