...
def make_dataloaders_from_csv(csv_path, parts = ['lhemi', 'rhemi', 'sub'], 
...
class CondProjector3D(nn.Module):
    """
    Projects concatenated part-latents (e.g., 3 parts * 3 latent ch = 9 ch)
    down to k channels using 1x1x1 conv, optionally with norm+activation.

    Input:  cond_lat  [B, in_ch, h, w, d]
    Output: cond_k    [B, k,     h, w, d]
    """
    def __init__(self, in_ch: int, out_ch: int, use_norm: bool = True, act: str = "silu"):
        super().__init__()
        self.proj = nn.Conv3d(in_ch, out_ch, kernel_size=1, bias=True)

        self.norm = nn.GroupNorm(num_groups=min(8, out_ch), num_channels=out_ch) if use_norm else nn.Identity()
...
def known_and_overlap_from_list(masks, r_seam, device):
    ...
    for m in m_bool:
        ...
        #m_sum += dilate3d(m.to(torch.int16), r_seam)

    known_mask   = (m_sum >= 1).float().clamp(0,1)
    ...
    for m in m_bool:
        ...
        #m_sum += m
        m_sum += dilate3d(m.to(torch.int16), r_seam)
    overlap_mask = (m_sum >= 2).float().clamp(0,1)
...
def build_proto1_from_batch(batch, bg_val: float = -1.0,
    ...
    I = (mL + mR + mC).clamp(0, 1)
    H = dilate3d(I, r_seam).clamp(0, 1)


    masks = [mL, mR, mC]
    device = batch["lhemi_mask"].device
    known, O = known_and_overlap_from_list(masks, 1, device)
    #O_dilated = dilate3d(O, 1).clamp(0, 1)
    O = dilate3d(O, 1).clamp(0, 1)
    B = torch.full_like(known, 1)

    # Preserve only non-overlapping interior
    #I_safe = (I * (1 - O_dilated)).clamp(0, 1)
    #I = erode3d(I, 1).clamp(0, 1)
    I_safe = (I * (1 - O)).clamp(0, 1)

    # Unknown-ish region (editable) excluding seam band and safe interior
    U = ((B - I_safe)).clamp(0, 1)
    ...
    xL = pL * mL * (1 - O) + bg_val * O
    xR = pR * mR * (1 - O) + bg_val * O
    xC = pC * mC * (1 - O) + bg_val * O
    ...
    cond_img = torch.cat([xL, xR, xC], dim=1) # [B,3,H,W,D]
    masks = dict(mL=mL, mR=mR, mC=mC, xL=xL, xR=xR, xC=xC, O=O, coarse=coarse, I_safe=I_safe, B=B, U=U, H=H)
    if save_example == True:
        save_masks_nifti(masks, outdir="ckpts/masks_debug_0120", prefix="coarse1", idx=0,make_uint8=False)
...
def sample_ldm_proto1(
    data_loader, autoencoder, unet, cond_proj, 
    scheduler, device,
    latent_shape,
    ...
    zL_int = autoencoder.encode_stage_2_inputs(xL_int.to(device))
    zR_int = autoencoder.encode_stage_2_inputs(xR_int.to(device))
    zC_int = autoencoder.encode_stage_2_inputs(xC_int.to(device))

    # Build I_safe_lat3
    # resize each latent to match global latent grid (z_gt grid)
    target_hwd = latent_shape[-3:]  # (h,w,d)
    zL = F.interpolate(zL_int, size=target_hwd, mode="trilinear", align_corners=False)
    zR = F.interpolate(zR_int, size=target_hwd, mode="trilinear", align_corners=False)
    zC = F.interpolate(zC_int, size=target_hwd, mode="trilinear", align_corners=False)

    eps = torch.randn(latent_shape).to(device)
    t   = torch.randint(0, scheduler.num_train_timesteps, (latent_shape[0],), device=device).long().to(device)

    # build cond_lat correctly (you already do this part)
    # concatenate part-latents -> 9ch
    cond_lat9 = torch.cat([zL, zR, zC], dim=1)   # [B,9,h,w,d]
    cond_latk = cond_proj(cond_lat9)         # [B,k,h,w,d]
    ...
    for i, t in enumerate(timesteps):
        ...
        model_in = torch.cat([z, cond_latk], dim=1) # [B, 3+2, 12,16,12]n
        #print(cond_lat.shape, z.shape, model_in.shape)
...
def train_ldm(unet, train_loader, autoencoder, cond_proj, r_int = 2, r_seam = 5, bg_val = -1.0, ldm_epochs = 150,
    ...
    for epoch in range(n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            zC_int = autoencoder.encode_stage_2_inputs(xC_int.to(device))
            # Build I_safe_lat3
            B_lat = F.interpolate(B, size=z_gt.shape[-3:], mode="nearest")     # [B,1,12,16,12]
            B_lat3 = B_lat.expand(-1, z_gt.shape[1], -1, -1, -1)               # [B,3,12,16,12]

            # resize each latent to match global latent grid (z_gt grid)
            target_hwd = z_gt.shape[-3:]  # (h,w,d)
            zL = F.interpolate(zL_int, size=target_hwd, mode="trilinear", align_corners=False)
            zR = F.interpolate(zR_int, size=target_hwd, mode="trilinear", align_corners=False)
            zC = F.interpolate(zC_int, size=target_hwd, mode="trilinear", align_corners=False)
            ...
            with torch.autocast("cuda", enabled=torch_autocast):
                ...
                t   = torch.randint(0, scheduler.num_train_timesteps, (z_gt.shape[0],), device=z_gt.device).long()

                z_t = scheduler.add_noise(z_gt, eps, t)

                # concat-condition into UNet input
                unet_in = torch.cat([z_t, cond_latk], dim=1)  # [B, 3+k, h,w,d]

                optimizer_diff.zero_grad(set_to_none=True)

                # forward (adjust signature to your UNet)
                eps_hat = unet(unet_in, t)  # or unet(unet_in, t, context=...) depending on your code
        ...
        if epoch_loss / (step + 1) < unet_best_loss:
            ...
            torch.save(cond_proj.state_dict(), os.path.join(outdir, "COND_PROJ_best.pt"))
            sample_ldm_proto1(train_loader, autoencoder, unet, cond_proj, scheduler, device, latent_shape = latent_shape,
        ...
        _save_ckpt(os.path.join(outdir, "UNET_last.pt"), unet, opt=optimizer_diff, scaler=scaler, epoch=epoch+1, global_step=None, 
                    extra={"train_loss": epoch_loss_list, "scale_factor": float(scale_factor)})
        torch.save(cond_proj.state_dict(), os.path.join(outdir, "COND_PROJ_last.pt"))
...
def main():
    ...
    ap.add_argument("--cond_proj_ckpt", default="", help="Conditioner weights (optional)")
    ap.add_argument("--cond_proj_k", type=int, default=4, help="Token dim")
    ...
    k = args.cond_proj_k
    cond_proj = CondProjector3D(in_ch=9, out_ch=k, use_norm=True, act="silu").to(device)

    if args.cond_proj_ckpt != "":
        cond_proj.load_state_dict(torch.load(args.cond_proj_ckpt, map_location=device))
    ...
    unet = DiffusionModelUNet(
        spatial_dims=3,
        in_channels=3+k, # default: 3 + num cond channel
    ...
    if args.stage in ["ldm", "both"]:
        train_ldm(unet, train_loader, autoencoder, cond_proj, r_int = args.ldm_r_int, r_seam = args.ldm_r_seam, ldm_epochs = args.ldm_epochs,
