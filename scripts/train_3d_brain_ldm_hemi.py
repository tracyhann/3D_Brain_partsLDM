...
def map_hemi_part_id(value: Any) -> int:
    """
    Normalize hemisphere labels to IDs:
      left_hemi -> 0
      right_hemi / right_hemi_mirror -> 1
    """
    if value is None or (isinstance(value, float) and np.isnan(value)):
        raise ValueError("part value is missing/NaN")
    ...
    if sval in mapping:
        return mapping[sval]
    raise ValueError(f"Unrecognized hemisphere part label: {value}")
...
def make_dataloaders_from_csv(csv_path, conditions = ['age', 'sex', 'vol', 'group'], train_transforms = None,
    ...
    for idx, row in df.iterrows():
        ...
        for c in conditions:
            if c == "part":
                sample[c] = map_hemi_part_id(row[c])
            else:
                sample[c] = row[c]
...
def _save_ckpt(
    ...
    global_step: int,
    epoch: int,
    conditioner: Optional[torch.nn.Module] = None,
...
def _load_ckpt_into_unet(
    ...
    cond_ok = True
    if conditioner is not None and ckpt.get("conditioner_state_dict") is not None:
        c_missing, c_unexpected = conditioner.load_state_dict(
            ckpt["conditioner_state_dict"], strict=False
        )
        print(
            "[resume] loaded conditioner: "
            f"missing={len(c_missing)} unexpected={len(c_unexpected)}"
        )
        cond_ok = (len(c_missing) == 0 and len(c_unexpected) == 0)
    elif conditioner is not None and ckpt.get("conditioner_state_dict") is None:
        cond_ok = False

    # Full resume (optimizer/scaler) only works when model topology matches.
    # If topology changed, keep loaded weights but continue with fresh optimizer state.
    opt_ok = True
    ...
    elif opt is not None and ckpt.get("optimizer") is None:
        opt_ok = False
    ...
    model_ok = (len(missing) == 0 and len(unexpected) == 0)
    if not (model_ok and cond_ok and opt_ok):
        print(
            "[resume] incompatible for exact resume; falling back to warm-start "
            "(step=0, fresh optimizer history). Use --ldm_ckpt for warm-start explicitly."
        )
        return 0, 0, {}
    return gs, ep, extra
...
def _extract_part_ids(batch: Dict[str, Any], part_key: str, device) -> torch.Tensor:
    """
    Read hemisphere id from batch and return int tensor [B].
    Expected ids: 0=left, 1=right.
    """
    if part_key not in batch:
        raise KeyError(
            f"Missing conditioning key '{part_key}' in batch. "
            f"Available keys: {sorted(list(batch.keys()))}"
        )
    part_ids = torch.as_tensor(batch[part_key], device=device).long().view(-1)
    return part_ids


class HemisphereConditioner(nn.Module):
    """
    Simple embedding-based conditioner for hemisphere IDs.
    part_id: 0=left, 1=right.
    Returns context tokens [B,1,d_model] for cross-attention UNet.
    """
...
def eval_ldm_loss_only(
    *,
    ...
    for i, batch in enumerate(val_loader):
        ...
        part_ids = _extract_part_ids(batch, part_key, device)
        cond_lat = conditioner(part_ids)
        with torch.no_grad():
...
def eval_ldm_fast(
    *,
    ...
    # ---------- B) DDIM sampling (fast, manual loop for version compatibility) ----------
    ddim = DDIMScheduler(
        num_train_timesteps=1000,
        schedule="scaled_linear_beta",
        beta_start=0.0015,
    ...
    z0 = torch.randn(latent_shape, device=device)

    # conditioning ids (half left, half right)
    if n_parts < 2:
        eval_part_ids = torch.zeros((eval_n,), device=device, dtype=torch.long)
    else:
        n_left = eval_n // 2
        n_right = eval_n - n_left
        eval_part_ids = torch.cat(
            [
                torch.zeros((n_left,), device=device, dtype=torch.long),
                torch.ones((n_right,), device=device, dtype=torch.long),
            ],
            dim=0,
        )
    eval_cond_lat = conditioner(eval_part_ids)
    ...
    for t in ddim.timesteps:
        ...
        with autocast(device_type="cuda", enabled=amp_enabled):
            eps = unet(z, timesteps=t_batch, context=eval_cond_lat)
    ...
    eval_part_ids_cpu = eval_part_ids.detach().cpu().numpy().tolist()
    for i in range(x_gen_cpu.shape[0]):
        arr = x_gen_cpu[i, 0].numpy()
        p = int(eval_part_ids_cpu[i])
        nib.save(
            nib.Nifti1Image(arr, affine),
            os.path.join(step_dir, f"sample_{i:03d}_part{p}.nii.gz"),
        )
...
def main():
    ...
    ap.add_argument("--part_key", default="part", help="CSV/data key for hemisphere ID (0=left,1=right).")
    ap.add_argument("--n_parts", type=int, default=2, help="Number of hemisphere classes. Default: 2 (left/right).")
    ap.add_argument("--cond_dim", type=int, default=64, help="Cross-attention conditioning dimension.")
    ...
    ap.add_argument("--conditioner_ckpt", default="", help="Optional standalone conditioner checkpoint path.")
    ...
    train_loader, val_loader, test_loader = make_dataloaders_from_csv(
        args.csv,
        conditions=[args.part_key],
    ...
    unet = DiffusionModelUNet(
        ...
        cross_attention_dim=args.cond_dim,
    ...
    unet.to(device)
    conditioner = HemisphereConditioner(n_parts=args.n_parts, d_model=args.cond_dim).to(device)

    scale_factor = None

    if args.ldm_ckpt != "":
        _load_ckpt_into_unet(unet, args.ldm_ckpt, device, conditioner=conditioner)
    ...
    if args.conditioner_ckpt != "":
        cstate = torch.load(args.conditioner_ckpt, map_location=device, weights_only=False)
        cstate = cstate.get("state_dict", cstate)
        conditioner.load_state_dict(cstate, strict=False)
