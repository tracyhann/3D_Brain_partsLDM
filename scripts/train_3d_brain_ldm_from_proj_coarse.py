...
def sample_ldm_proto1(
    ...
    z_coarse = autoencoder.encode_stage_2_inputs(coarse.to(device)) # [B,3,12,16,12]
    z_coarse = z_coarse.flatten(2).transpose(1,2)   # [B,N,3]
    cond_latk = cond_proj(z_coarse) * 2.0       # [B,N, 256]
...
def train_ldm(unet, train_loader, autoencoder, cond_proj, r_int = 2, r_seam = 5, bg_val = -1.0, ldm_epochs = 150,
    ...
    for epoch in range(n_epochs):
        ...
        if epoch_loss / (step + 1) < unet_best_loss:
            ...
            sample_ldm_proto1(train_loader, autoencoder, unet, cond_proj, inferer, scheduler, device, latent_shape = latent_shape,
...
def main():
    ...
    ap.add_argument("--cond_proj_k", type=int, default=256, help="CrossAttn dim")
    ...
    k = args.cond_proj_k # cross attention dim
    cond_proj = nn.Linear(3, k).to(device)
