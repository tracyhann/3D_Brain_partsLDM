...
class DiffusionInferer(Inferer):
    ...
    def __call__(  # type: ignore[override]
        ...
        if pairs is not None:
            noisy_image = torch.cat([noisy_image, pairs], dim=1)
        diffusion_model = (
    ...
    def sample(
        ...
        for t, next_t in progress_bar:
            ...
            if pairs is not None:
                model_input = torch.cat([model_input, pairs], dim=1)
                model_output = diffusion_model(
                    model_input, timesteps=torch.Tensor((t,)).to(input_noise.device), context=conditioning
                )
            else:
                model_output = diffusion_model(
