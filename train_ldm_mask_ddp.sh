...
CONFIG="${CONFIG:-${CONFIG_PATH:-configs/whole_brain_maskLDM_spacing1p5.json}}"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-scripts/train_3d_ldm_mask_ddp.py}"
...
OUT_PREFIX="${OUT_PREFIX:-${CFG_PREFIX:-whole_brain_mask_UNET}}"
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/train_mask_ddp_${SLURM_JOB_ID:-manual}_node${NODE_RANK}_$(date +%Y%m%d_%H%M%S).log"
