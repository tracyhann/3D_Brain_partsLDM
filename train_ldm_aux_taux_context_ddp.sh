...
CONFIG="${CONFIG:-${CONFIG_PATH:-configs/whole_brain_aux_taux_spacing1p5_CONTEXT.json}}"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-scripts/train_3d_brain_ldm_aux_taux_context_ddp.py}"
...
OUT_PREFIX="${OUT_PREFIX:-${CFG_PREFIX:-whole_brain_aux_taux_context_UNET}}"
...
RUN_DIR="${OUTDIR_ABS}/${OUT_PREFIX}_${OUT_POSTFIX}"
mkdir -p "${RUN_DIR}"

LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs}"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/train_aux_taux_context_ddp_${SLURM_JOB_ID:-manual}_node${NODE_RANK}_$(date +%Y%m%d_%H%M%S).log"
